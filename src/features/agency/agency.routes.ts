...
import { create, del, download, edit, getAll, getFields, update, upload } from './agency.controller'

const routes = (server: Server): Router => {
    const contextPrefix = createUrlContext('agency')
    ...
    router.del('/:id', checkAuthorized, checkRole([roles.MASTER]), del)
